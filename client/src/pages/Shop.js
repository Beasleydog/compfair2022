    ...
    return (
      <div className="mt-4 ml-4">
        <button
          ...
          className={
            stars < req && !unlockData[type].includes(id)
        ...
        >
          <img
            src={src}
            className={`h-[150px] w-[150px] bg-[#151617] rounded-[50%] shadow ${
              profilePicture[type] == id
...
function ShopSection({ contents, title }) {
  return (
    <div className="w-full flex flex-col rounded-lg bg-[#1a1c1f] shadow">
      <div className="text-[40px] text-center text-white w-full">{title}</div>
      <div className="flex justify-left gap-[20px]">{contents}</div>
    </div>
  );
}
